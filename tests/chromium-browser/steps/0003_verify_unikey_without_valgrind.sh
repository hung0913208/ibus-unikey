...
EXPORT="GTK_MODULES=gail:atk-bridge OOO_FORCE_DESKTOP=gnome GNOME_ACCESSIBILITY=1 QT_ACCESSIBILITY=1"
...
if which chromium-browser &> /dev/null; then
	...
	error "can't detect the right application for testing"
fi
	...
	for METHOD in ${METHODs[@]}; do
		...
		for CASE in $(ls -1c $SUITE); do
			...
			if [ $EXT = "py" ]; then
				...
				if ! exec_on_test_machine "DISPLAY=:0 GTK_IM_MODULE=ibus $EXPORT python ~/$CASE $METHOD $APP"; then
