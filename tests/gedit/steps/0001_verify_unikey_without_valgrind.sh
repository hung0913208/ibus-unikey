...
elif ! exec_on_test_machine_without_output "mkdir -p /home/$(username)/ibus-unikey/build"; then
...
for SPEC in ${SPECs[@]}; do
	...
	elif ! exec_on_test_machine_without_output --add-arg '-Y' "DISPLAY=:0 ibus engine Unikey"; then
		error "can't config ibus to use with Unikey"
	fi

	for STEP in {0..11}; do
		if [ $(exec_on_test_machine --add-arg '-Y' "DISPLAY=:0 ibus engine") != 'Unikey' ]; then
			if [[ $STEP -gt 10 ]]; then
				error "can't choose engin Unikey"
			else
				sleep 10
			fi
		else
	...
	for METHOD in ${METHODs[@]}; do
		DEFAULT=$(exec_on_test_machine --add-arg '-Y' "DISPLAY=:0 gsettings get org.freedesktop.ibus.engine.unikey input-method")

		if [ "$DEFAULT" != "$METHOD" ]; then
			if ! exec_on_test_machine --add-arg '-Y' "DISPLAY=:0 gsettings set org.freedesktop.ibus.engine.unikey input-method $METHOD"; then
		...
		for CASE in $(ls -1c $SUITE); do
			...
			if [ $EXT = "py" ]; then
				...
				if ! exec_on_test_machine --add-arg '-Y' "DISPLAY=:0 python ~/$CASE $METHOD"; then
